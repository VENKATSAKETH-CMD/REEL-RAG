...
def answer_reel_question(session: Session, reel: Reel, user_message: str, top_k: int = 5) -> str:
    """
    High-level helper: retrieve chunks for a reel and answer a question using OpenAI chat.
    
    EMBEDDING CONSISTENCY:
    - Uses embed_text() to encode the query (same function as worker uses for indexing)
    - This ensures query embeddings match indexed chunk embeddings
    
    REQUIREMENTS:
    - PostgreSQL + pgvector for vector search
    - OPENAI_API_KEY for chat completions (not for embeddings; embeddings are via embed_text)
    
    PRODUCTION SAFETY:
    - Catches rate limits, API errors, and auth failures
    ...
    Args:
        ...
        reel: The Reel object to search chunks from
        user_message: The user's question
        top_k: Number of chunks to retrieve (default: 5)
    
    Returns:
        A string answer generated by OpenAI using the retrieved chunks as context
    
    Raises:
        RuntimeError: If configuration is missing or unrecoverable API error occurs
    """
    ensure_postgres_for_rag()

    openai_api_key = os.getenv("OPENAI_API_KEY")
    if not openai_api_key:
        raise RuntimeError("OPENAI_API_KEY not set. Required for chat completions in answer_reel_question.")
    
    client = OpenAI(api_key=openai_api_key)

    # 1) Encode user question using embed_text (SAME function as worker uses for chunks)
    # This ensures we search using vectors generated by the same algorithm.
    query_embedding = embed_text(user_message)

    # 2) Retrieve similar chunks using pgvector distance search
    chunks = search_reel_chunks_pgvector(
        session=session,
        reel_id=reel.id,
    ...
    # 3) Build context text from retrieved chunks
    if chunks:
    ...
    # 4) Generate answer using OpenAI chat model with comprehensive error handling
    system_prompt = (
    ...
    try:
        completion = client.chat.completions.create(
            ...
            messages=messages,
            temperature=0.2,
            max_tokens=500  # Enforce reasonable token limit
        )
        return completion.choices[0].message.content.strip()
    
    except RateLimitError as e:
        # API rate limit hit (quota exceeded, too many requests, etc.)
        error_detail = str(e)[:100]
        raise RuntimeError(f"OpenAI API rate limited. Please try again in a few moments. ({error_detail})")
    
    except APIError as e:
        # Other API errors (auth failures, server errors, model not available, etc.)
        error_detail = str(e)[:100]
